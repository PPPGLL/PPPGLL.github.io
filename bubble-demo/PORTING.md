...
- HDR 文件由浏览器端 Radiance RGBE/RLE 解码器读入并上传为 `RGBA16F`。
  超出半浮点有限范围的太阳高光会先钳制到 `65504`，避免 `Inf/Inf` 在 ACES 中产生黑色 NaN 斑块。

## 已移植功能

- Belcour/Barla 三层薄膜模型与 256×256 LUT
- 最新版公园原始 4K HDR 环境图、白炉测试、泡泡独立输出和 ACES 输出
...
- 进入编辑工具时冻结场景并清零速度；只有动态模拟/浏览模式推进物理
- 双击放置随机泡泡簇、长按拖框多选、选中轮廓、整组拖拽/复制/缩放/删除
- 单个选中泡泡的半径、膜厚、流动、表面张力和阻尼独立调整
- 重力缩放、竖直环境气流和随机深度分散
- 单次/逐层景深近似、完整设备像素渲染比例与连接区域法线融合控制
- 最终合成后执行 FXAA，补偿 WebGL2 单采样浮点离屏缓冲的轮廓锯齿
